...
//for add user open machine modal
//parameter(attender / machine)
function open_machine(type)
{
    if (type == 'attender')
    {
        $('#machinehead').html(ui_string['machine']);
        $('#user_machine').modal();
    } else
    {
        $(".ads_Checkbox_machine").val('');
    }
...
function open_add_role(tp, id)
{
    ...
    if (tp == 'add')
    {
        ...
        $('#grouprole').html('<button type=\'button\' class=\'btn btn-theme-inverse btn_width right bottom-gap\' onclick="return validation(\'addRole\')">' + ui_string['confirm'] + '</button>');
    ...
    if (tp == 'update')
    {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data['applyToHirarchy'] == '1') {
                    $('#applyToHirarchy1').attr('checked', "checked");
                }
                if (data['applyToHirarchy'] == '0') {
                    $('#applyToHirarchy2').attr('checked', "checked");
                }
