...
foreach ($cursor as $doc) {
    ...
    $action_buttons .= '<a data-original-title="Delete" onclick="delete_license(this.id,\''.$doc['userId'].'\')" id="' . $id . '" data-id="' . $id . '" class="btn btn-default btn-sm" title="Delete">';
    ...
    if (isset($doc['active_status'])) {
        ...
        $status = '<a data-status="' . $data_status . '" data-id="' . $id . '" onclick="change_license_status(this.id)" id="status-' . $id . '" style="border-color: ' . $color . '; color: ' . $color . ';" class="btn btn-default btn-link" title="' . $data_status . '">
    ...
    $date_of_expiry ='N/A';
    $userId ="N/A";
    if(isset($doc['userId']) && !empty($doc['userId'])){
        $userName = get_resource_by_id(array('id'=>$doc['userId'],'fields'=>'name'));
        if($userName['success']=='true' && !empty($userName['data'][0]))
        {
            $userId = $userName['data'][0]['name'];
        }
    }
    ...
    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons,'status'=>$status,"user_avatar"=>$user_avatar,'category_profile'=>$category_profile,'user_type'=>$userType,'password'=>$password,'lno'=>$lno,'license'=>$license,'date_of_expiry'=>$date_of_expiry,'userId'=>$userId);
