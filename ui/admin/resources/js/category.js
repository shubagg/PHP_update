            ...
            $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if(data['success']=='true')
                            {
                                ...
                                $("#model_des").html(ui_string['category_success']);
                                $('#success_modal').modal();
                            ...
                            else
                            {
                                ...
                                {
                                     $("#model_head").html(ui_string['notconfirm']);
                                    $("#model_des").html(ui_string['category_unsuccess']);
                                    $('#success_modal').modal();
                                    setTimeout(function(){ $('#success_modal').modal("toggle"); },1000)
                                }
       ...
       case "updateCategory":
            ...
            $.ajax({
                        ...
                        success: function(data) {
                            ...
                            {
                                $("#model_head").html(ui_string['confirm']);
                                $("#model_des").html(ui_string['category_update']);
                                $('#success_modal').modal();
                                //setTimeout(function(){ window.location=userDetailurl; },1000);
                                 setTimeout(function(){ location.reload(); },1000)
                            }
                            else
                            {
                                ...
                                {
                                    $("#model_head").html(ui_string['notconfirm']);
                                    $("#model_des").html(ui_string['category_update_unsuccess']);
                                    $('#success_modal').modal();
                                    setTimeout(function(){ $('#success_modal').modal("toggle"); },1000)
                                }
                            }                      
...
function open_add_category(tp,id)
{
    ...
    if(tp=='add')
    {
       // alert(tp);
        ...
        $('#cat_id').val('0');
         $('#root_parent_id').val('');
        $('#cathead').html(ui_string['add_category']);
        $('#groupbut').html('<button type=\'button\' class=\'btn btn-theme-inverse btn_width right bottom-gap\' onclick="return validation(\'addCategory\')"><i class=\'glyphicon glyphicon-plus-sign\'></i>'+ui_string['confirm']+'</button>');
            ...
            $.ajax({
                        ...
                        success: function(data) {
                           ...
                           // console.log(data);
                            ...
                            {
                                $("#model_head").html(ui_string['confirm']);
                                $("#model_des").html(ui_string['category_delete']);
                                $('#success_modal').modal();
                                setTimeout(function(){ location.reload(); },1000)
                            }
                            else
                            {
                                $("#model_head").html(ui_string['notconfirm']);
                                $("#model_des").html(ui_string['category_delete_unsuccess']);
                                $('#success_modal').modal();
                                setTimeout(function(){ $('#success_modal').modal("toggle"); },2000)
                            }
                        },
