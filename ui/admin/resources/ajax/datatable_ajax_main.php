...
{  
    ...
    {
        array_push($cs,$cd);
        //array_push($cs,array('category'=>array('$elemMatch'=>$cd)));    
    }
    $qarray=array();
    $cs=array('category'=>array('$exists'=>true,'$in'=>$cs));
...
foreach ( $cursor as $doc ) {
    ...
    $action_buttons.='<a data-original-title="Edit" onclick="go_to(\'user.php\',\'id=\'+$(this).attr(\'data-id\'))" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="">';
    ...
    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons,'user_status'=>$status,"user_avatar"=>$user_avatar,'category_profile'=>$category_profile);
