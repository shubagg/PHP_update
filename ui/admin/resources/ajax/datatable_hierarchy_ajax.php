...
else
{
            ...
            if($currentId=='569f7faa7c3d68011e3c9869')
            {
                $where=array('parentId'=>"{$currentId}",'status'=>array('$ne'=>'10'),'user_type'=>array('$ne'=>'super admin')) ;
            }
            else
            {
                $managerOf=get_managers_category(array('userId'=>$currentId));
                ...
                if(sizeof($managerOf)>0)
                {
                    $where=array('category'=>array('$exists'=>true,'$in'=>$managerOf),'status'=>array('$ne'=>'10'),'user_type'=>array('$ne'=>'super admin'));
                }
            }
...
foreach ( $cursor as $doc ) {
   ...
   /* if(isset($doc['department'])){
    ...
    /*$category_profile="N/A";
    $get_user_categories=curl_post("/get_user_category",array("category_ids"=>implode("|",$doc['category']),"code"=>'profile')); 
    if(!empty($get_user_categories['data'])){
       $category_profile=implode(" | ",array_filter(explode(" | ",$get_user_categories['data'])));
    }*/

    /*$category_area="N/A";
    $get_user_categories1=curl_post("/get_user_category",array("category_ids"=>implode("|",$doc['category']),"code"=>$area));
    if(!empty($get_user_categories1['data'])){
        $category_area=implode(" | ",array_filter(explode(" | ",$get_user_categories1['data'])));
    }*/
   ...
   /* $manager="N/A";
    $get_user_categories2=curl_post("/get_user_category_manager",array("category_ids"=>$doc['manager']));
    if(!empty($get_user_categories2['data'])){
       $manager=$get_user_categories2['data']; 
    }*/
    

    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons,'user_status'=>$status,"user_avatar"=>$user_avatar,'category_profile'=>$category_profile,'category_area'=>$category_area,'manager'=>$manager,'address'=>$address,'companyAddress'=>$companyAddress,'department'=>$department,'designation'=>$designation,'googleID'=>$googleID);
