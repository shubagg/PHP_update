...
if ($userInfo['success'] == 'true' && $userInfo['data'] != "") {
    ...
    {
        $where['role'] = array('$ne' => '5cf4c668518be4001e000032');
    }
}
...
foreach ($cursor as $doc) {
    ...
    $encrypt  = encrypt_decrypt($id, 'encrypt', 'nikky', '');
    $checkbox = '<span class="checkbox" data-color="red">
            <input id="ch_' . $id . '" onclick="checkBoxChecked(this.id)" name="numbers[]" value="' . $encrypt['data'] . '" class="check_box check_box_enroll" type="checkbox"> <label></label>
    </span>';
    ...
    $action_buttons .= '<a data-original-title="Edit" onclick="go_to(\'updateUser\',$(this).attr(\'data-id\'))" id="data_table_1-' . $id . '" data-id="' . $encrypt['data'] . '" class="btn btn-default btn-sm" title="Edit">';
    $action_buttons .= '<i class="fa fa-pencil"></i>';
    $action_buttons .= '</a>&nbsp;';

    $action_buttons .= '<a data-original-title="Delete" onclick="delete_users_temp(this.id)" id="data_table_1-' . $id . '" data-id="' . $encrypt['data'] . '" class="btn btn-default btn-sm" title="Delete">';
    ...
    $status = '<a title="'.ucfirst($data_status).'" onclick="update_status(this.id)" data-status="' . $data_status . '" data-id="' . $encrypt['data'] . '" id="status-' . $id . '" style="border-color: ' . $color . '; color: ' . $color . ';" class="btn btn-default btn-link">
