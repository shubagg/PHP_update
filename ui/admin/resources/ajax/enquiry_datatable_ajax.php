...
else
{
            ...
            if($currentId=='569f7faa7c3d68011e3c9869')
            {
                $where=array('status'=>array('$ne'=>'10')) ;
            }
            else
            {
                $managerOf=get_managers_category(array('userId'=>$currentId));
                ...
                if(sizeof($managerOf)>0)
                {
                    $where=array('category'=>array('$exists'=>true,'$in'=>$managerOf),'status'=>array('$ne'=>'10'));
                }
            }
...
foreach ( $cursor as $doc ) {
    ...
    $action_buttons.='<a data-original-title="Delete" onclick="delete_enquiry_temp(this.id)" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="">';
    ...
    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons,'user_status'=>$status,"user_avatar"=>$user_avatar,'category_profile'=>$category_profile,'category_area'=>$category_area,'manager'=>$manager);
