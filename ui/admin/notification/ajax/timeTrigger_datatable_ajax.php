...
foreach ( $cursor as $doc ) {
    ...
    $action_buttons.='<a data-original-title="Edit" onclick="go_to_time_trigger(\''.$_GET['id'].'\',this.id)" id="'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="Edit">';
        $action_buttons.='<i class="fa fa-pencil"></i>';
    $action_buttons.='</a>&nbsp;';
  
    $action_buttons.='<a data-original-title="Delete" onclick="delete_time_trigger_temp(this.id)" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="">';
    ...
    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons,'triggerId'=>$triggerId,'status'=>$status);
