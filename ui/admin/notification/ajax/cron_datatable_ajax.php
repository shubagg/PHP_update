...
foreach ( $cursor as $doc ) {
    ...
    $id=$doc['_id']->{'$id'};
     ...
     $checkbox='<span class="checkbox" data-color="red">
            ...
            $action_buttons.='<a onclick="start_cron(\''.$doc['cronId'].'\',\''.$doc['fileName'].'\')" data-original-title="'.$ui_string['timeTrigger'].'" class="" title="'.$ui_string['timeTrigger'].'" style="cursor:pointer;">';
                $action_buttons.=$ui_string['start'];
            $action_buttons.='</a>&nbsp; | ';

             $action_buttons.='<a onclick="stop_cron(\''.$doc['cronId'].'\',\''.$doc['fileName'].'\')" data-original-title="'.$ui_string['timeTrigger'].'" class="" title="'.$ui_string['timeTrigger'].'"  style="cursor:pointer;">';
                $action_buttons.=$ui_string['stop'];
            $action_buttons.='</a>&nbsp; | ';

             $action_buttons.='<a href="timeTrigger?id='.$doc['cronId'].'" target="_blank" data-original-title="'.$ui_string['timeTrigger'].'" class="" title="'.$ui_string['timeTrigger'].'">';
                $action_buttons.=$ui_string['timeTrigger'];
            $action_buttons.='</a>&nbsp;';
          
            $action_buttons.='</span>';
    
    
    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons,'status'=>$status,"user_avatar"=>$user_avatar,'user_type'=>$userType,'password'=>$password);
