...
foreach ( $cursor as $doc ) {
    ...
    $action_buttons.='<a data-original-title="Edit" onclick="go_to(\'EditTrigger\',$(this).attr(\'data-id\'))" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="Edit">';
        $action_buttons.='<i class="fa fa-pencil"></i>';
    $action_buttons.='</a>&nbsp;';
  
    $action_buttons.='<a data-original-title="Delete" onclick="delete_trigger(this.id)" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="">';
    ...
    $subject=$doc['subject'];
    $msg=$doc['msg'];
    if(isset($doc['subject_'.$_SESSION['user']['lang']]))
    {
        $subject=$doc['subject_'.$_SESSION['user']['lang']];
    }
    if(isset($doc['msg_'.$_SESSION['user']['lang']]))
    {
        $msg=$doc['msg_'.$_SESSION['user']['lang']];
    }

    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons,'mid'=>$mid,'smid'=>$smid,'eid'=>$eid,'subject'=>$subject,'msg'=>$msg);
