function validate1()
{
    ...
    {
        $('#acc_email_err').html(ui_string['ent_email']);
        $("#acc_email").focus();
        return false;
    }
    ...
    else
    {
        ...
        var datastring = "name="+name+"&domain="+domain+"&_csrf="+_csrf+"&id="+id+"&username="+username+"&type="+type+"&password="+password+"&email="+email+"&url="+url+"&port="+port+"&action=account_add";
        $.ajax({
            type: "POST",
