...
//include("formui.php");
 ...
 $formdetail=get_form_by_id(array('id'=>0,'status'=>'1','nor'=>1));
 $formdetail=$formdetail['data']['0'];
 $formjson=json_encode($formdetail['field']);
$_GET['formid'] = $formdetail['id'];
?>
...
<?php
 include_once(include_admin_extensions_template("feedback_form","feedback_form","formAttributes"));


 include_once(include_admin_extensions_template("feedback_form","feedback_form","formdetail"));
    ...
    $('form').ajaxForm({
        ...
        complete: function(xhr) {
            ...
            {       
                    ...
                    $('#success_modal').modal();
                    setTimeout(function(){ window.location=site_url+'admin/feedbackform/listing?type=form_listing&formtype=pendingform&job=3'; },1000);
            }
            else
            {         
                   setTimeout(function(){ window.location=site_url+'admin/feedbackform/listing?type=form_listing&formtype=pendingform&job=3'; },1000);
