  ...
  <div class="modal-content">
    ...
    <input type="hidden" id="signatureMediaId" value="">
        ...
        <div class="col-xs-12" style="margin-top:15px; padding:0;">
            <div class="js-signature" style="text-align: center;" data-width="200" data-height="200" data-border="1px solid #1ABC9C" data-background="#fff" data-line-color="#081bf5" data-auto-fit="true"></div>      
        </div>
       <div style="clear:both;"></div>
    </div>
    <div class="modal-footer">
...
<div class="modal fade in" id="galleryModal" role="dialog" data-backdrop="static" data-keyboard="false"  data-width="700">
  
    <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <h4 class="modal-title" id="galleryTitle"></h4>
    </div>
    <div class="modal-body new_panel">
       <div id="galleryImages" style="width:100%;height:500px;text-align:center;"></div>
    </div>
  
  <div class="modal-footer">
            <button type="button" class="btn btn-default"  onclick="previous_image()">PREVIOUS</button>
...
  if(signatureMediaId==1){ signId=signId-1; }
  ...
  $('#canvasimg').hide();
  var signHtml='';
  signHtml+='<div class="videoThumbs " id="imageData-'+signId+'">';
      ...
      signHtml+='<div class="sign_bg123" onclick="openGallery(\'Signature\',\''+signId+'\',\'signatureList_'+rFieldId+'\')" style="cursor:pointer;">';
        signHtml+='<img data-width="300" style="width: 200px;height: 200px;border:1px solid" src="'+data+'"/>';
      signHtml+='</div>';
      signHtml+='<div>';
...
function openGallery(title,id,fieldId)
{
  currentImageId=id;
  currentFieldId=fieldId;
  $('#galleryTitle').html(title);

  var cwidth=$('.'+fieldId+' #imageData-'+id+' img').attr('data-width');
  
  var imageSrc=$('.'+fieldId+' #imageData-'+id+' img').attr('src');
  /*var width="";
  if(cwidth>=500){ width="width:100%;"; }
  var cheight=$('.'+fieldId+' #imageData-'+id+' img').attr('data-height');
  if(cheight && cheight>500){ width="width:280px;"; }
  $('#galleryImages').html('<img src="'+imageSrc+'" style="'+width+'" width="100%" height="100%"/>');*/
  var width="";
  var height="100%";
  if(cwidth>=500){ width="width:500px;"; }
  var cheight=$('.'+fieldId+' #imageData-'+id+' img').attr('data-height');
  if(cheight && cheight>500)
    { width="width:280px;"; height="height:500px;";}
  else if(cheight && cheight<500)
  {
    height="";
  }
  
  $('#galleryImages').html('<img src="'+imageSrc+'" style="'+width+'" width="'+width+'" height="'+height+'"/>');
  $('#galleryModal').modal();
}

function next_image()
{
  var nextDiv=$('.'+currentFieldId+' #imageData-'+currentImageId).next('div').attr("id");
  if(nextDiv)
  {
    var id=nextDiv.split('-');
    currentImageId=id[1];
    var imageSrc=$('.'+currentFieldId+' #imageData-'+currentImageId+' img').attr('src');
    $('#galleryImages').hide();
    var cwidth=$('.'+currentFieldId+' #imageData-'+currentImageId+' img').attr('data-width');
    var width=cwidth;
    if(cwidth>=500){ width="width:500px;"; }
    var cheight=$('.'+currentFieldId+' #imageData-'+currentImageId+' img').attr('data-height');
    var height = cheight;
   if(cheight && cheight>500){ width="width:280px;"; }
    $('#galleryImages').html('<img src="'+imageSrc+'" style="'+width+'" width="'+width+'" height="'+height+'"/>');

    $('#galleryImages').show("slide", { direction: "right" }, 200);
  }
}

function previous_image()
{
  var previousDiv=$('.'+currentFieldId+' #imageData-'+currentImageId).prev('div').attr("id");
  if(previousDiv)
  {
    var id=previousDiv.split('-');
    currentImageId=id[1];
    var imageSrc=$('.'+currentFieldId+' #imageData-'+currentImageId+' img').attr('src');
    $('#galleryImages').hide();
    var cwidth=$('.'+currentFieldId+' #imageData-'+currentImageId+' img').attr('data-width');
  ...
  if(cheight && cheight>500){ width="width:280px;"; }
    $('#galleryImages').html('<img src="'+imageSrc+'" style="'+width+'" width="100%" height="100%"/>');
    $('#galleryImages').show("slide", { direction: "left" }, 200);
  }
}
</script>
