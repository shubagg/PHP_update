...
if(isset($_GET['formtype']) && $_GET['formtype']=="pendingform") {
  $getPendingdata=curl_post('/get_job_by_id',array('id'=>$_GET['jobId'],"smid"=>"3","allStatus"=>"true"));
...
if($jobStatus['success']=='true')
{
  ...
  $current_assignee = $jobStatus['userid'];
}
//pr($getPendingdata); die;
...
{
  $_GET['formid'] = $getPendingdata['form_id']; 
}
$status=ex_getformstatus($getPendingdata['status']);
//echo $status; die;
...
$current_owner='';
if($current_assignee) {
  $current_owner_details = curl_post("/get_resource_by_id", array("id" => $current_assignee));
      ...
      <div >
        ...
        <form id="addnewform" class="form-horizontal" method="post" action="<?php echo extensions_url()?>aintu_form_job/ui/admin/submit.php"  enctype="multipart/form-data" >
          ...
          <div class="row">
            <div class="col-md-6 col-xs-12">
              <div class="form-group">
                <label for="inputOne"><?php echo $ui_string['form_status'];?> - <?php echo $status . ' with '. $current_owner;?></label>
              </div>
            </div>
            <div class="col-md-6 col-xs-12 ">
              <div class="form-group pull-right">
          ...
          <div class="row">
            <div class="col-md-12 col-xs-12">
                <?php 
                      ...
                      $currentId=1;
                      $disablestatus='off';
                      if(($_GET['formtype']=="pendingform") && ($getPendingdata['userid']==$userId))
                      { 
                        $disablestatus='on'; 
                      ...
                      if(isset($formdetail) && $formdetail!='') {
                        $forceEditMode = 'off'; 
                        if(check_user_permission("job","forms","edit")!=1)
                        {
                          $disablestatus = 'off';
                          $forceEditMode = 'on'; 
                        }
                        
                        displayForm($formdetail,$formValueJSON,$disablestatus,$forceEditMode);  
                      }
...
(function() {
    ...
    $('form').ajaxForm({
        ...
        complete: function(xhr) {
            ...
            window.location=site_url+"admin/form_job_report";
