...
if (!empty($searchTermsAny)) {
    ...
    $where=array('type'=>'partnerwithevenzo');
...
$categories = array();
$getCategoriesData = get_categories(array());
$getCategoriesData = $getCategoriesData['data'];

foreach ($getCategoriesData as $key => $value) 
{
...
foreach ( $cursor as $doc ) {
    ...
    if(isset($categories[$doc['venuetype']]))
    {
        $doc['venuetype'] = $categories[$doc['venuetype']]['title'];
    }
    else
    {
          $doc['venuetype'] =  'N/A';
    ...
    $action_buttons.='<a data-original-title="Delete" onclick="delete_data_record(this.id)" id="data_table_1-'.$id.'" data-id="'.$id.'" data-type="contactus" class="btn btn-default btn-sm" title="">';
    ...
    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons);
