...
if (!empty($searchTermsAny)) {
    ...
    $where=array('type'=>'3','ProductType'=>'venue');
...
foreach ( $cursor as $doc ) {
    ...
    $checkbox="";
    $detail="";
    $detaillist=json_decode($doc['detail'],true);
    foreach ($detaillist as $key => $value) {
        $detail .=$key."-".$value.", ";
    }
    
    $other_fields=array('checkbox'=>$checkbox,'date'=>$datetime,'title'=>$getProductTitle,'username'=>$username,'action'=>$action_buttons,'status'=>$status[$doc['venueStatus']],"user_avatar"=>$user_avatar,"description"=>$detail);
