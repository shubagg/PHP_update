...
if (!empty($searchTermsAny)) {
    ...
    $where=array('type'=>'3','ProductType'=>'event');
...
foreach ( $cursor as $doc ) {
    ...
    $username=get_resource_by_id(array('id'=>$doc['userId'],'fields'=>'email'));
    $username=$username['data'][0]['email'];

    $datetime=date("Y-m-d H:i:s a",$doc['createdOn']->sec);
    $checkbox="";
    $details=json_decode($doc['detail'],true);
    $bookedfor=date("Y-m-d H:i:s a",$doc['bookedfor']/1000);
    $bookedStatus='Pending';
    if($doc['bookedStatus']=='1')
    {
        $bookedStatus='Paid';
    }
    $detailsList="";
    foreach ($details as $key => $value) {
        $detailsList .="Name=".$value['name'].", Quantity=".$value['quantity'].'</br>';
    }
    $other_fields=array('checkbox'=>$checkbox,'date'=>$datetime,'title'=>$getProductTitle,'username'=>$username,'details'=>$detailsList,'bookedfor'=>$bookedfor,'bookedStatus'=>$bookedStatus,'action'=>$action_buttons,'user_status'=>$status,"user_avatar"=>$user_avatar);
