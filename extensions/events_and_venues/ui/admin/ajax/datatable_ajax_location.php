...
$where=array('groupId'=>$_GET['groupId']);
$qarray=array();
...
foreach ( $cursor as $doc ) {
    ...
    $action_buttons.='<a data-original-title="Edit" onclick="go_to_popup(\'amenitiesPopup\',\''.$doc['countryId'].'\',\'id=\'+$(this).attr(\'data-id\'))" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="Edit">';
        $action_buttons.='<i class="fa fa-pencil"></i>';
    $action_buttons.='</a>&nbsp;';

    $action_buttons.='<a data-original-title="view" onclick="go_to_city_popup(\'cityPopup\',\''.$doc['stateId'].'\',\'id=\'+$(this).attr(\'data-id\'))" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="view">';
        $action_buttons.='<i class="fa fa-eye"></i>';
    $action_buttons.='</a>&nbsp;';

    $action_buttons.='<a data-original-title="Delete" onclick="delete_data_temp(this.id)" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="delete">';
    ...
    $locationImage=$imageData['media']['1'][$id][0]['mediaName'];

    if($locationImage!=''){$img_url=media_url().'images/'.$locationImage;}  
    else{ $img_url=ui_media_url().'image_not_available_400.png'; }  
    
    $user_avatar="<img src='$img_url' width='50' height='50'/>";

    $checkbox="";
    $other_fields=array('checkbox'=>$checkbox,'action'=>$action_buttons,'user_status'=>$status,"user_avatar"=>$user_avatar);
