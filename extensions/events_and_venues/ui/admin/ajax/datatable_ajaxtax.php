...
if (!empty($searchTermsAny)) {
	...
	if($user_type == 'hotel')
	{
		...
		$where['hotelUserId'] = $_SESSION['user']['user_id']; //$product_id;
...
foreach ( $cursor as $doc ) {
    ...
    $action_buttons.='<a data-original-title="Edit" onclick="go_to_taxpopup(\'id=\'+$(this).attr(\'data-id\'))" id="'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="Edit">';
    ...
    $action_buttons.='<a data-original-title="Delete" onclick="delete_taxdata_temp(this.id)" id="data_table_1-'.$id.'" data-id="'.$id.'" class="btn btn-default btn-sm" title="">';
