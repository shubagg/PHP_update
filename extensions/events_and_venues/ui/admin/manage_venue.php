...
/*else if ($_SESSION['user']['user_type']=='hotel'){ // this else is added  by Umesh


	
	$queryExecute = select_mongo('product',array('userId'=>$_SESSION['user']['user_id']),array());
	$proudctId = add_id($queryExecute);
	$proudctId=$proudctId[0];
	$_GET['id'] = $product_id = $proudctId['id'];
	
	$product=get_product_by_id(array("id"=>$product_id));
	$product=$product['data'][0];
	$Eventid=$product['id'];
	$hiddenVal = 'hotel';
...
var showFields=[];
var popUpItemId='';
var coursePath= site_url+'/ui/admin/course';
...
.amenity ul{
				...
				<header class="panel-heading">
					<div class="row">
						<div class="col-md-6 margn_tp_7">
						      <h3><strong>Manage</strong> Hotel</h3>
						</div>
					</div>
					</header>
					<div class="panel-body panel_bg_d">
						<form id="add_event_data" class="form-horizontal" method="post" action="" enctype="multipart/form-data">
                         <input type="hidden" id="chekcHotel" value="<?php echo $hiddenVal; ?>">
						
						
						<div class="form-group ">
							<label class="control-label remove_bg col-md-2">Hotel *</label>
							<div class="col-md-10">
							
								<select id="hotel" name="hotelUserId"  data-check-valid="blank" data-error-show-in="ehotel" data-error-setting="2" data-error-text="Please select hotel" class="form-control required_field add_event_data editUser" value="" placeholder="">
									<option value="">Select Hotel</option>
									 
									 <?php 
										$get_user = curl_post("/get_hotel_user",array('id'=>$_SESSION['user']['user_id'])); 
										foreach ($get_user['data'] as $get_uservalue) { 
									 ?>
									<option id="<?php echo $get_uservalue['id']; ?>" <?php if($get_uservalue['id']==$product['hotelUserId'])
									{ echo "selected"; } ?> value="<?php echo $get_uservalue['id']; ?>"><?php echo ucfirst($get_uservalue['name']); ?></option>
									<?php } ?>
									
									
									</select>
							     <span id="ehotel" class="error"></span>
						    </div>
						</div>
						...
						<!--<div class="form-group ">
							<label class="control-label remove_bg col-md-2">Username *</label>
							<div class="col-md-10">
								<input type="text" id="email" name="email" data-check-valid="blank,email" data-error-show-in="eemail" data-error-setting="2" data-valid-email-error="Please enter correct email" data-error-text="Please enter email." class="form-control required_field add_event_data editUser" value="<?php echo $product['email']; ?>" placeholder="" >
							     <span id="eemail" class="error"></span>
						    </div>
						</div>
						<div class="form-group ">
							<label class="control-label remove_bg col-md-2">Password *</label>
							<div class="col-md-10">
								<input type="password" id="pass" name="password" data-check-valid="blank" data-error-show-in="epassword" data-error-setting="2" data-valid-email-error="Please enter correct email" data-error-text="Please enter password." class="form-control required_field add_event_data editUser" value="<?php echo $product['password']; ?>" placeholder="" >
						 ...
						 <div class="form-group ">
							...
							<div class="col-md-10">
								 <input type="text" id="name" name="title" value="<?php echo $product['title']; ?>" data-check-valid="blank" data-error-show-in="eTitle" data-error-setting="2" data-error-text="Please enter title" class="form-control required_field add_event_data editUser" value="" placeholder="">
								 <input type="hidden" id="type" name="type" value="venue">
								 <input type="hidden" name="id" id="eventsId" value="<?php echo $Eventid; ?>">
								 <input type="hidden" name="userId" id="userId" value="<?php echo $product['userId']; ?>">
						...
						<div class="form-group ">
							<label class="control-label remove_bg col-md-2">Phone No. *</label>
							<div class="col-md-10">
								<input type="text" id="phone" name="phone" data-check-valid="blank,numeric" data-error-show-in="ephone" data-error-setting="2" data-valid-numeric-error="Please enter only numeric value" data-error-text="Please enter phone no." class="form-control required_field add_event_data editUser" value="<?php echo $product['phone']; ?>" placeholder="" >
							     <span id="ephone" class="error"></span>
						    </div>
						</div>
						...
						<div class="form-group ">
							<label class="control-label remove_bg col-md-2">Booking Request Email *</label>
							<div class="col-md-10">
								<input type="text" id="username" name="booking_req_email" data-check-valid="blank,email" data-error-show-in="eusername" data-error-setting="2" data-valid-email-error="Please enter correct email" data-error-text="Please enter booking request email." class="form-control required_field add_event_data editUser" value="<?php echo $product['booking_req_email']; ?>" placeholder="" >
							     <span id="eusername" class="error"></span>
						    </div>
						</div>
						...
						<div class="form-group ">
							...
							<span id="eanimities" class="error"></span>
						    </div>
						...
						<div class="form-group ">
							<label class="control-label remove_bg col-md-2">Best For *</label>
							<div class="col-md-10">
								<input type="text" id="facility" name="facility" value="<?php echo $product['facility']; ?>" data-check-valid="blank" data-error-show-in="efacility" data-error-setting="2" data-error-text="Please enter facilities" class="form-control required_field add_event_data editUser" value="" placeholder="">
							     <span id="efacility" class="error"></span>
						    </div>
						</div>
						<div class="form-group ">
						...
						<div class="form-group">
							   ...
							   <div class="col-md-10">
							      ...
							      <!-- hidden id of crop -->
							      <input type="file" style="display:none;" name="product_image" id="product_image" data-hidden-cropdata-id="hiddenCropData" data-hidden-cropdata-type-id="hiddenCropType" data-width="800" data-height="600" data-returnfunction="" class="form-control cropimage" />
						...
						<div class="form-group ">
                            ...
                            <button type="reset" class="btn btn-inverse right left-gap" onclick="cancel_user_registration('addUser')" ><i class="glyphicon glyphicon-chevron-right"></i> Reset</button>
                            <button  onclick="return validation('add_event_data')" type="button" class="btn btn-theme-inverse right"><i class="glyphicon glyphicon-circle-arrow-right"></i> Submit</button>
						</form>
					</div>
					</section>
					</div>
	...
	<?php
		...
		//echo success_fail_message_popup();
	?> 

	<?php 
	//echo "test";
		get_banners_popup(array("mid"=>"16","smid"=>"2")); 
	?>
