...
function verify_license_status($data) {
    ...
    if ($check['success'] == 'true') {
        ...
        if($data['type'] == 'registerClientUser') {
            if(!empty($data['userInfo'])) {
                $userInfo = json_decode($data['userInfo'], true);
                if(!empty($userInfo['business_email'])) {
                    $details = get_resource_by_id(array('business_email' => $userInfo['business_email']));
                    if ($details['success'] != 'true') {
                        $output = insert_mongo('user', $userInfo);
                        if ($output['n'] == '0') {
                            $id = $lparam['_id']->{'$id'};
                            return array('data' => $id, 'error_code' => '55012', 'success' => 'true');
                        } else {
                        ...
                        return array('data' => !empty($details['data'][0]['id']) ? $details['data'][0]['id'] : '', 'error_code' => '55012', 'success' => 'true');
                    }
                } else {
                    return array('data' => "Not Submitted", 'error_code' => '55011', 'success' => 'false');
                }
...
{ 
    ...
    if(isset($data['status']) && $data['status'] == 'del')
    {
        ...
        $cond['userId'] = '';
        $updateuser = update_mongo('user',array('license'=>'', 'status' => '0'),array('_id'=> new MongoId($getstatusdata['userId'])));
    }
    ...
    if($update['n'] == 1)
    {
        if($cond['active_status'] == '1') {
            update_mongo('user',array('status' => '1'),array('_id'=> new MongoId($getstatusdata['userId'])));
        } else {
            update_mongo('user',array('status' => '0'),array('_id'=> new MongoId($getstatusdata['userId'])));
        }
