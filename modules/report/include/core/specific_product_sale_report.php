...
function get_specific_product_sale_report($data)
{
    ...
    if(!empty($tmp))
    {
        ...
        foreach ($tmp as $key=>$value) 
        {
            ...
            {
                $totalOrder = count_product_sale(array('start'=>$value[0],'end'=>$value[1]));
                $orderInfo   = get_product_sale_data(array('start'=>$value[0],'end'=>$value[1]));
                array_push($rowchart, $value[1]);
                array_push($rowchart, $totalOrder);
            }
            else
            {
                $totalOrder = count_product_sale(array('start'=>$value,'end'=>$value));
                $orderInfo   = get_product_sale_data(array('start'=>$value,'end'=>$value));
...
function count_product_sale($data)
{
    $condition=array();
    $dt = $data['start']. " 00:00";
...
function get_product_sale_data($data)
{
        logger("19",'',$data,5);   
        $condition=array();
        $dt = $data['start']. " 00:00";
