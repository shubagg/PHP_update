...
<div id="<?php echo $popupId; ?>" class="modal fade container">
                <div class="modal-header">
                        ...
                        <h4 class="modal-title"><i class="glyphicon glyphicon-user"></i>Assign</h4>
                </div>
                <!-- //modal-header-->
...
<span class="checkbox all_Sel slect_aal_btn pull-right" data-color="red" id=>
 ...
 <div class="clearfix"></div>
   ...
   <div class="row">
               <span class="error" id="assign_error"></span>
    <div class="col-md-12">
...
function genericUsersPopupTemplate(userid,id)
{
    $("#template_id").val(id);
    $("#md_full_user_scheduler_1").modal();
}
var all_checked_array_scheduler = [];
var all_checked_array_values_scheduler = [];
function check_box_user_template(id)
{
...
function template_popup_data(id)
{
    setloader();
    var checked_ids_template = [];
    $(".sel_checkbox_template:checked").each(function () {
        checked_ids_template.push($(this).val());
    });
    var template_id = $("#template_id").val();
    var user_id_template = checked_ids_template;
    $.ajax({
        url: admin_ui_url + "controlTower/ajax/assign_template.php",
        data: "user_id=" + user_id_template + "&template_id=" + template_id,
        type: "POST",
        success: function (suc)
        {
            unloading();
            $("#dismiss_scheduler").click();
...
{  
    ...
    {
    var assign = [];
    $(".check_box_assign:checked").each(function () {
    assign.push($(this).val());
        });
//        if(assign=='')
//        {
//            $('#assign_error').html(ui_string['assign_error']);
//            return false;
//
//        }
        if(popUpItemId)
        {
            ...
            setTimeout(function(){ 
                var selected_robot_users = new Array();
                    $(".robot_assign_check_box:checked").each(function() {
                        selected_robot_users.push($(this).val());
                    });
            /******************************/
                $.ajax({
                url:"<?php echo site_url(); ?>webservices/enroll",
                data:"userId="+selected_robot_users.toString()+"&itemId="+popUpItemId+"&categoryId="+catData+"&mid="+mid+"&smid="+smid+"&proj_group_id="+group_id,
                type:"POST",
                async:false,
                success:function(suc)
                        {
                            ...
                            {
                                $.ajax({
                                    url:"<?php echo site_url(); ?>webservices/copy_data",
                                    data:"userId="+selected_robot_users.toString()+"&asid="+popUpItemId+"&id="+robotid,
                                    type:"POST",
                                    success:function(data){
                                        $(".check_box_assign").attr('checked', false);
                                        $('#<?php echo $popupId; ?>').modal('toggle');
